...
Everything below is orchestrated locally inside a single Claude Code session and ends in a printed report. **By default Review Council is report-only** — it never pushes commits or posts to a PR on its own. The one exception is opt-in: when `pr_comments.enabled` is set (default **off**), an added step (Step 6.6) posts a single **review-digest comment** to the PR — but only after an explicit human confirmation, never automatically, and it may offer to open a draft PR if none exists. See the **Posting to PRs** section in `README.md`. Note the data egress this implies: when Codex, Google, or Perplexity are enabled, the gathered review context (diff, file contents, etc.) is sent to those third-party tools/APIs — only Claude (the native subagent) stays fully local. When posting is enabled, the digest (findings + decisions) is also sent to GitHub.
...
11. **Post digest** (Step 6.6, gated on `pr_comments.enabled`, default off) — Optional and human-confirmed: after the report, walks the findings with the author (fix / defer / won't-fix — the same walk Step 7 reuses), composes a single **review-digest** comment (*what was found & by whom · what the council decided · what was fixed*), and posts/updates it on the PR via `scripts/rc-post.sh` (idempotent singleton, keyed by a hidden marker). Posts as the authenticated `gh` user by default (clearly marked automated), or as a bot when `pr_comments.bot_token_env` names a set token var. If no PR exists it offers to open a draft. Fail-soft — never blocks or mutates the review.
12. **Capture Gate** (Step 7, gated on `settings.learn`) — Record-only: after the report, walks the surviving findings with the author (tackle / skip / skip-all — reusing Step 6.6's walk when posting ran), distills a *skip with a generalizable reason* into a Suppression (keyed by the judge's canonical fingerprint) or a Convention, and — human-confirmed only — appends it to `.review-council/learnings.md` via `scripts/rc-learn.sh`. This is the write side of the learnings loop; what it writes here, step 2 (Recall learnings) reads on the next run.
...
review-council/
  ...
  scripts/            Config reader (rc-config.sh), static-analysis runner (rc-static-scan.sh), learnings writer (rc-learn.sh), PR-digest posting engine (rc-post.sh), shared timeout lib (rc-lib-timeout.sh), Codex + Google provider-dispatch state machine (rc-invoke-provider.sh)
...
**PR-comment posting (`pr_comments.*`, `scripts/rc-post.sh`) inherits the same discipline.** `pr_comments.bot_token_env` names an env var that `rc-post.sh` reads via indirect expansion, so it is validated as a strict POSIX identifier (`valid_envname`) at the reader and re-checked before any `eval` in `rc-post.sh` (defence in depth). The **token value never lives in config** — only the *name* of the env var that holds it does. The **target PR is always derived from local git** (`gh pr view` on the current branch, or the draft just created), **never from config** — a reviewed repo must not be able to redirect where the bot posts. And posting is fail-soft and human-gated: it never auto-posts and never blocks or mutates the review.
