...
2. If `agy` returns **empty or malformed output** — the known `agy -p` quirk of exiting **0 with no stdout** in a non-TTY, usually a cold-start artifact — **retry `agy` once**. The second, warm call almost always returns a valid review. Do not go to `gemini` yet.
3. Fall back to `gemini` only when `agy` cannot be salvaged: `agy` is **absent**, **hard-fails** (auth/quota — retrying won't help), **times out** (retrying would double the budget), or its **step-2 retry also returns empty/malformed**.
